// src/app/api/ops/users/[id]/delete/route.ts
...
export async function POST(
  ...
  if (error) {
    console.error('[ops/users/delete] deleteUser failed:', error);
    return Response.json(
  ...
  if (target.email) {
    await sendEmail({
      to: target.email,
      subject: 'Your UbuntuScamBank account has been deleted',
      text: accountDeletedEmail(target.username),
    });
  }
